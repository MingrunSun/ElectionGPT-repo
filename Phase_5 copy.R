...
rsconnect::deployApp('/Users/sunmingrun/Desktop/AI Project/Testing/publish')


# Melt the dataframe to long format
data<- read_excel("panel_election_results2.xlsx")
...
extended_data <- melted_data %>%
  ...
  mutate(abb=state)%>%
  mutate(Predicted_party=ifelse(Percent_byStateParty>=0.5, "Republican", "Democratic")) %>%
  left_join(df, by = "abb") %>%
  mutate(fips = as.character(fips)) %>%
  mutate(new_variable = runif(n(), min = 0, max = 50))# Convert FIPS to character type
...
server <- function(input, output) {
  ...
  USA_map <- reactive({
    ...
    filter(extended_data, Type %in% input$voicechoice) %>%
      ...
      mutate(hover = paste(state, '<br>', 
                           ...
                           "Date:", Date, "<br>",
                           "Type:", Type, "<br>",
                           "Predicted Party:", Predicted_party))
  })
  ...
  output$Plot_Map <- renderPlotly ({
    ...
    fig <- fig %>% add_trace(
      z = ~new_variable, text = ~hover, locations = ~state,
      color = ~new_variable, colors = 'Purples'
    )
